...
# For hg + q1 - q2 + case
def search_for_q2_insertions(sites_dict, chain_dict, outfile):

	print "Searching for evidence, case 0."
	print "Writing results to {}.".format(outfile)
	...
	for chain_id in chain_ids:
		...
		for site in sites_dict[chain_id]:
			...
			if gapless_percentage > THRESHOLD:
				print "Evidence found! Gapless percentage: {}".format(gapless_percentage)
				found_evidence.add('\t'.join(site))
			else:
				print "Gapless percentage: {}. Insufficient.".format(gapless_percentage)
	...
	for chain_id in sites_dict.keys():
		...
		for site in sites_dict[chain_id]:
			...
			for line in chain[1:]:
				...
				#if on last chain line, continue
...
def main():
	...
	if mode == 0:
		search_for_q2_insertions(sites_dict, chain_dict, outfile)
	elif mode == 1:
		double_insertion(sites_dict, chain_dict, outfile)	
	elif mode == 2:
		search_for_q2_deletions(sites_dict, chain_dict, outfile)
